...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "gsc_api.py check failed with exit $LASTEXITCODE" }
    python scripts/gsc_api.py queries --days 28 --export (Join-Path $outDir "gsc-queries-api.json")
    if ($LASTEXITCODE -ne 0) { throw "gsc_api.py queries failed with exit $LASTEXITCODE" }
    python scripts/gsc_api.py pages --days 28 --export (Join-Path $outDir "gsc-pages-api.json")
    if ($LASTEXITCODE -ne 0) { throw "gsc_api.py pages failed with exit $LASTEXITCODE" }
}
finally {
    Pop-Location
}
...
try {
    ...
    "GBP sync skipped or failed - see log" | Out-File (Join-Path $outDir "gbp-reviews-sync.log") -Encoding utf8
}
finally {
    Pop-Location
}
...
if ((Test-Path $serpConfig) -and (Test-Path $outreach)) {
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) { throw "serp_intel.py failed with exit $LASTEXITCODE" }
        if (Test-Path "state\serp-reports") {
            Copy-Item -Path "state\serp-reports" -Destination (Join-Path $outDir "serp-reports") -Recurse -Force
        }
    }
    catch {
        "Serper run failed - check SERPER_API_KEY" | Out-File (Join-Path $outDir "serper-error.log") -Encoding utf8
    }
    finally {
        Pop-Location
    }
...
if (Test-Path $queriesPath) {
    try {
        ...
        if ($q.queries) {
            $top = $q.queries | Sort-Object { [double]$_.impressions } -Descending | Select-Object -First 10
        ...
        $lines += "## GSC parse note"
        $lines += "Could not parse gsc-queries-api.json for table output."
    }
}
